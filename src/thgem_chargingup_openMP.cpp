...
void kaptonChargesVector(int charge, double kapton, Double_t zend, double *vectorCharges,const int nSlices,double normstep){
	...
	if(charge==1) {
	  //	  cout<<"Ion stopped in kapton in slice "<<i<<" with ";
	  *(vectorCharges+nSlices-1)+=normstep;
	}
	else if(charge==-1) {
	  //cout<<"Electron stopped in kapton in slice "<<i<<" with ";
	  *(vectorCharges+nSlices-1)-=normstep;	
...
int main(int argc, char * argv[]) {
  ...
  //./gem_chargingup nSlices nprimary_elec vgem gas_name numberOfIterations Pressure Temperature
  //ex ./gem_chargingup 20 1 400 xe 100 760 293
...
//    float zhigh=0.1;
	...
	//  std::vector<Double_t> SliceNumberOfCharges(nSlices);
	if(debug) cout << "Define branches " << endl;
	...
	//double sum = std::accumulate(std::begin(gain), std::end(gain), 0.0);
	double sum=0;
